-- Create user-specific tables for the health app (Simplified version)
...
-- 9. Create basic RLS policies (without complex UUID casting)
-- For now, we'll create simple policies that allow all operations
-- You can refine these later based on your authentication needs

-- Allow all operations on user_habits (you can restrict this later)
CREATE POLICY "Allow all operations on user_habits" ON user_habits
    FOR ALL USING (true);

-- Allow all operations on user_interventions
CREATE POLICY "Allow all operations on user_interventions" ON user_interventions
    FOR ALL USING (true);

-- Allow all operations on intervention_habits
CREATE POLICY "Allow all operations on intervention_habits" ON intervention_habits
    FOR ALL USING (true);

-- Allow all operations on daily_habit_entries
CREATE POLICY "Allow all operations on daily_habit_entries" ON daily_habit_entries
    FOR ALL USING (true);

-- Allow all operations on intervention_feedback
CREATE POLICY "Allow all operations on intervention_feedback" ON intervention_feedback
    FOR ALL USING (true);
