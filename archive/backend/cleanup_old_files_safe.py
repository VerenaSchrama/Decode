#!/usr/bin/env python3
"""
SAFE cleanup script that preserves the Alisa Vita book vectorstore
Removes only CSV and intervention-related files, keeps the InFlo book vectorstore
"""
...
def identify_files_to_delete_safe():
    ...
    # We need to be very careful here - only delete intervention-related vectorstores
    ...
    # OLD intervention vectorstore files
    intervention_vectorstore_files = [
        # Note: We keep chroma.sqlite3 as it might contain InFlo book data
    ]
    
    # Check which files actually exist
    for file_path in csv_files + json_files + intervention_vectorstore_files:
...
def check_database_migration_status():
    ...
    try:
        ...
        print(f"   New intervention vectorstore: {'✅ Exists' if vectorstore_exists else '❌ Missing'}")
...
def main():
    """Main safe cleanup function"""
    
    print("🧹 SAFE Cleanup: Preserve InFlo Book Vectorstore")
    print("=" * 60)
    
    # Check if InFlo vectorstore is intact
    print("🔍 Checking InFlo book vectorstore...")
    ...
    # Check if database migration was completed
    ...
    print("✅ Both InFlo vectorstore and database migration verified!")
    ...
    print(f"   📁 data/vectorstore/database_chroma/ (New intervention vectorstore)")
    ...
    response = input("\nProceed with safe cleanup? (y/N): ").strip().lower()
    if response != 'y':
    ...
    print(f"   ✅ Database vectorstore preserved")
    print(f"   💾 Space saved: {space_saved:.2f} MB")
    
    print(f"\n📋 What's preserved:")
    print(f"   📚 InFlo book vectorstore (for scientific context)")
    print(f"   🗄️  Database with interventions and habits")
    print(f"   🔍 New intervention vectorstore")
    
    print(f"\n📋 Next steps:")
    print(f"   1. Test your API endpoints")
    print(f"   2. Test your mobile app")
    print(f"   3. Verify InFlo context is still working")
    print(f"   4. Remove backup directory if everything works: rm -rf data/cleanup_backup/")
